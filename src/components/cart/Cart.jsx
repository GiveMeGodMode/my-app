...
const Cart = (props) => {
    const onAddOverlay = async (obj) => {
        try {
            ...
            if (findOverlay) {
                axios.delete(
                    `https://637f91ed5b1cc8d6f949a8dd.mockapi.io/cart/${findOverlay.id}`
                );
                props.setOverlayItems((over) =>
                    over.filter((item) => item.myId !== obj.myId)
                );
            } else {
    ...
    const onAddFav = async (obj) => {
        try {
            ...
            if (findFavorites) {
                axios.delete(
                    `https://637f91ed5b1cc8d6f949a8dd.mockapi.io/favorites/${findFavorites.id}`
                );
                props.setFavorites((over) =>
                    over.filter((item) => item.myId !== obj.myId)
                );
            } else {
                const { data } = await axios.post(
